...
void solver_tweak2(solver_t* sp, MatchObj* mo, int order, sip_t* verifysip) {
    ...
    mo->sip = tweak2(xy, Nxy,
                     ...
                     (int)solver_field_width(sp),
                     (int)solver_field_height(sp),
                     refradec, mo->nindex,
                     indexjitter, qc, Q2,
                     sp->distractor_ratio,
...
void solver_verify_sip_wcs(solver_t* solver, sip_t* sip) { //, MatchObj* pmo) {
    ...
    nindexes = solver->nindexes;
    for (i=0; i<nindexes; i++) {
        index_t* index = solver->indexes[i];
...
static void try_all_codes(const pquad_t* pq,
...
static void check_scale(pquad_t* pq, solver_t* s) {
    double dx, dy;
...
static void check_inbox(pquad_t* pq, int start, solver_t* solver) {
    int i;
    double Ax, Ay;
...
static void print_inbox(pquad_t* pq) {
    int i;
    debug("[ ");
 ...
 */
static void add_stars(const pquad_t* pq, int* field, int fieldoffset,
...
void solver_run(solver_t* solver) {
    ...
    num_indexes = solver->nindexes;
    {
        ...
        pquads = calloc(numxy * numxy, sizeof(pquad_t));

        /* We maintain an array of "potential quads" (pquad_t) structs, where
        ...
        if (solver->startobj) {
            ...
            for (field[B] = 0; field[B] < solver->startobj; field[B]++) {
                for (field[A] = 0; field[A] < field[B]; field[A]++) {
                    pquad_t* pq = pquads + field[B] * numxy + field[A];
                    pq->fieldA = field[A];
                    pq->fieldB = field[B];
        ...
        for (newpoint = solver->startobj; newpoint < numxy; newpoint++) {
            ...
            if (solver->max_wall_time && solver->max_wall_time < get_wall_time() - solver->start_twall) {
            ...
            for (i = 0; i < num_indexes; i++) {
                ...
                for (field[A] = 0; field[A] < newpoint; field[A]++) {
                    ...
                    if (solver->cancel && *solver->cancel)
                        goto quitnow;
                    if (solver->quit_now)
                        goto quitnow;
                }
            ...
            for (field[A] = 0; field[A] < newpoint; field[A]++) {
                for (field[B] = field[A] + 1; field[B] < newpoint; field[B]++) {
                    ...
                    for (i = 0; i < (size_t)solver->nindexes; i++) {
                        int dimquads;
                        index_t* index = solver->indexes[i];
                        if ((pq->scale < minAB2s[i]) ||
                            (pq->scale > maxAB2s[i]))
            ...
            if ((solver->maxquads && (solver->numtries >= solver->maxquads))
                ...
                || solver->quit_now || (solver->cancel && *solver->cancel))
                break;
        }

    quitnow:
        for (i = 0; i < (size_t)(numxy*numxy); i++) {
            pquad_t* pq = pquads + i;
            free(pq->inbox);
            free(pq->xy);
        }
...
static void resolve_matches(kdtree_qres_t* krez, const double *field_xy,
    ...
    int jj, thisquadno;
    MatchObj mo;
    unsigned int star[DCMAX];

    assert(krez);
    // exit(1);
    for (jj = 0; (unsigned)jj < krez->nres; jj++) {
        double starxyz[DCMAX*3];
        double scale;
        double arcsecperpix;
        tan_t wcs;
        int i;
        ...
        if (thisquadno >= solver->index->nquads) {
            ANERROR("Requested quadid %i, but number of quads is %i", thisquadno, solver->index->nquads);
            assert(thisquadno < solver->index->nquads);
        }

        for (int i = 0; i < solver->index->dimquads; i++) {
            star[i] = solver->index->quads[thisquadno * solver->index->dimquads + i];
        }
...
void solver_set_default_values(solver_t* solver) {
    ...
    solver->logratio_tokeep = HUGE_VAL;
    solver->max_cpu_time = 3600 * 1000;
    solver->max_wall_time = 3600 * 1000;
