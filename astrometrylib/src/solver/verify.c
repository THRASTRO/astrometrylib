    ...
    if (vf->do_dedup) {
        ...
        if (!fake_match) {
            for (i=0; i<mo->dimquads; i++) {
                assert(mo->field[i] >= 0);
                assert(mo->field[i] < (unsigned)v->NTall);
...
static void verify_apply_ror(verify_t* v,
    ...
    if (vf->do_uniformize) {
        ...
        verify_get_uniformize_scale(index_cutnside, mo->scale, (int)fieldW, (int)fieldH, &uni_nw, &uni_nh);
...
void verify_uniformize_field(const double* xy,
    ...
    sz = calloc(sizeof(int), nw * nh);
    list = malloc(nw * nh * sizeof(int*));
    for (i = 0; i < (nw * nh); i++) {
...
double verify_star_lists_ror(double* refxys, int NR,
    ...
    NBx = (int)(ceil((double)W / sqrt(W*H) * sqrt(NB)));
    NBy = (int)(ceil((double)H / sqrt(W*H) * sqrt(NB)));
    NB = NBx * NBy;
