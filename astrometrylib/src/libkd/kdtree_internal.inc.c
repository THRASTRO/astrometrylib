...
#define FUNC_SUFFIX 
#include "kdtree_internal_dists.inc.c"
#undef PTYPE
#undef DISTTYPE
#undef FUNC_SUFFIX
        ...
        if (use_bboxes) {
            ...
            if (do_precheck && nodeid) {
                ...
                if (kd->splitdim)
                    ...
                    pdim = (int)kd->split.TTYPE[KD_PARENT(nodeid)];
                    pdim &= kd->dimmask;
                }
                if (TTYPE_INTEGER && use_tquery) {
                    if (isleftchild)
            ...
            if (TTYPE_INTEGER && use_tmath) {
                ...
                etype bblo[KDTREE_MAX_DIM], bbhi[KDTREE_MAX_DIM];
                int d;
                for (d=0; d<D; d++) {
...
static void convert_data(kdtree_t* kd, etype* edata, int N, int D, int Nleaf) {
    ...
    if (!ddata) {
        ANERROR("Failed to malloc %i x %i x %i\n", N, D, (int)sizeof(dtype));
        return;
    }
    kd->free_data = true;
    for (i=0; i<N; i++) {
...
kdtree_t* MANGLE(kdtree_build_2)
    ...
    int lnext, level;
    int maxlevel;
    dtype hi[KDTREE_MAX_DIM], lo[KDTREE_MAX_DIM];
    dtype* data = NULL;

    dtype nullbb[KDTREE_MAX_DIM];
    for (i=0; i<D; i++)
        nullbb[i] = 0;
    ...
    /* Parameters checking */
    if (!indata || !N || !D) {
        ANERROR("Data, N, or D is zero");
        return NULL;
    }
    /* Make sure we have enough data */
    if ((1 << maxlevel) - 1 > N) {
        ANERROR("Too few data points for number of tree levels (%i < %i)!", N, (1 << maxlevel) - 1);
        return NULL;
    }
    ...
    if ((options & KD_BUILD_SPLIT) &&
        ...
        TTYPE_INTEGER) {
        ANERROR("Currently you can't set KD_BUILD_NO_LR for int trees "
...
double MANGLE(kdtree_node_point_mindist2)
    ...
    if (!bboxes(kd, node, &tlo, &thi, D)) {
        ANERROR("Error: kdtree does not have bounding boxes!");
        return HUGE_VAL;
    }
    for (d=0; d<D; d++) {
        etype delta;
...
double MANGLE(kdtree_node_point_maxdist2)
    ...
    double d2 = 0.0;
    if (!bboxes(kd, node, &tlo, &thi, D)) {
        ANERROR("Error: kdtree_node_point_maxdist2_exceeds: kdtree does not have bounding boxes!");
        return false;
    }
    for (d=0; d<D; d++) {
...
double MANGLE(kdtree_node_node_maxdist2)
    ...
    if (!bboxes(kd1, node1, &tlo1, &thi1, D)) {
        ANERROR("Error: kdtree_node_node_maxdist2: kdtree does not have bounding boxes!");
        return false;
    }
...
double MANGLE(kdtree_node_node_mindist2)
    ...
    assert(kd1->ndim == kd2->ndim);
    if (!bboxes(kd1, node1, &tlo1, &thi1, D)) {
        ANERROR("Error: kdtree_node_node_mindist2: kdtree does not have bounding boxes!");
        return false;
    }
    if (!bboxes(kd2, node2, &tlo2, &thi2, D)) {
        ANERROR("Error: kdtree_node_node_mindist2: kdtree does not have bounding boxes!");
...
bool MANGLE(kdtree_node_node_maxdist2_exceeds)
    ...
    if (!bboxes(kd1, node1, &tlo1, &thi1, D)) {
        ANERROR("Error: kdtree_node_node_maxdist2_exceeds: kdtree does not have bounding boxes!");
        return false;
    }
...
bool MANGLE(kdtree_node_node_mindist2_exceeds)
    ...
    if (!bboxes(kd1, node1, &tlo1, &thi1, D)) {
        //ANERROR("Error: kdtree_node_node_mindist2_exceeds: kdtree does not have bounding boxes!");
        return false;
    }
...
static void nodes_contained_rec(const kdtree_t* kd,
    ...
    if (!bboxes(kd, nodeid, &tlo, &thi, D)) {
        ANERROR("Error: kdtree_nodes_contained: node %i doesn't have a bounding box", nodeid);
        return;
    }
...
void MANGLE(kdtree_nodes_contained)
    ...
    int D = kd->ndim;
    int d;
    ttype qlo[KDTREE_MAX_DIM], qhi[KDTREE_MAX_DIM];
